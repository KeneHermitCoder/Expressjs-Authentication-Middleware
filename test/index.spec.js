...
describe('express_authentication_middleware_basics', () => {
  ...
  it('Should generate an API key and add it to file', (done) => {
    keyStore(req, res);
    setTimeout(() => {
      utils.getKeysFromFile().then((data) => {
  ...
  it('Should validate header for protected routes', (done) => {
    ...
    agent
      ...
      .then(({ body: { apiKey } }) => {
        return Promise.all([
          agent.get('/tasks').set('x-api-key', apiKey),
          agent.post('/tasks').set('x-api-key', apiKey),
        ]);
      })
      .then((responseList) => {
  ...
  it('Should not require auth headers for unprotected routes', (done) => {
    agent = chai.request.agent(server);
    Promise.mapSeries(
      [agent.get('/tasks/3'), agent.get('/'), agent.post('/tasks')],
      (res) => res
    ).then((responseList) => {
